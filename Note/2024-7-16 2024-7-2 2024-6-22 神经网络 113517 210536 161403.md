...
​		1.多模态小模型：逆合成预测的关键之一在于模型对参与反应的分子性质的“理解”，而利用**文本信息**促进模型对分子的理解在生物领域大模型（BioT5,BioT5+,2024-5），在化学领域中的分子编辑（Multi-modal Molecule Structure-text Model for Text-based Retrieval and Editing，2022-12）已被证明**有效**。且可用的**数据集多**：如mol-instruction(面向分子的指令)、分子的IUPAC名称（BIOT5）、PubChemSTM（化学结构-文本对）、ChEBI- 20数据集（描述引导的分子设计任务）等。而**小模型训练的代价不高**，且因为**任务单一**（只做逆合成），所以更容易在单一任务取得较好效果。且大模型不容易发文章？
...
​	4.LOSS函数：为实现encoder对分子性质的理解，针对单模态数据（如带有 IUPAC 名称的分子 SELFIES，ZINC20 的分子 SELFIES等）采用**MLM**（借鉴BioT5）让模型预测被掩盖的某些embedding，；针对多模态数据（如分子-文本对），采用**对比学习**（借鉴CLIP， CLIP对齐不是太准确，这块前景很好。）。
...
​					（4）怎样从错误反应中学到有价值的东西（对比学习，learning from nosiy label这是两篇工作）【初步想法是设计一个非常简单的二分类任务，让模型区分对错】
...
​						   (3)怎样收集其他模型输出的错误反应（**单拿出来**，论文要有核心观点，不要杂糅），怎样收集；中可用的文本信息

在训练时不要提前泄露USPTO数据集的信息



#10.4更新：

1.使用Qformer代替encoder，分子表征学习框架冻结参数，作为分子模态的encoder（后续消融实验时可以换为MLP），Q-former采用多模态数据预训练。

2.Decoder使用PMSR的。对于Decoder的输出，似乎在采用分子表征学习框架时仍然可以采用现在的处理方法，毕竟encoder输出的也是embedding（attention怎么办）？

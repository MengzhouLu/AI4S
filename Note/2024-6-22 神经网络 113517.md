...
![c29e931cc09049feb2bff69377c94d56](C:\Users\gouwudang\Desktop\c29e931cc09049feb2bff69377c94d56.png)
...
1.借鉴VIT 把分子也分 patch切割？比如切割成环、链、官能团（因为学化学的时候好像就是这么分析性质的）（难以序列化表示的（比如环）可以直接用IUPAC命名等，待讨论），或者设计出一种特别的架构可以利用环等非序列化结构（比如设计出一种图神经网络分别提取这些patch的信息，然后输入进transformer中）

2.既然自然语言可以映射成embedding，那分子、原子、官能团、各种环等化学结构可不可以？  不同于分子指纹，这种embeding要有确切的语义空间————分子表征学习（Molecule representation learning，MRL）——https://blog.csdn.net/qq_40943760/article/details/121873051

3.化学反应中存在许多特例。学习特例：可以复用一个共享参数的模块 通过reshape输入 让模块关注不同的部分——temporal adaptation  或者使用标记告诉模型这是特例？

4.让attention更省内存:flash atten

5.可以做一些类似于adapter的工作 进一步优化已有模型 比如视频理解能力可以迁移到化学领域？（比如多步化学反应本身就是一个连续的过程）

6.CV和NLP领域的数据可以直接从互联网爬出来 可以不经过人类筛选，对真实世界抽样较好  但是化学反应式往往人们只记录那些有实际价值（在现实生活中得到应用）的 ，使数据集天然带有一种bias，这种bias是否会使模型不能具有创新型？利用那些被人类忽略的反应。

7.引入化学领域的domain knowledge(比如反应条件  官能团距离等)  反应类型已经有人做了  见T5的上一个论文

8.![image-20240702181024597](.\image-20240702181024597.png)
9.可以调整batch 使一个batch中只包括同类的反应与例外？


10.是不是可以先学反应预测（正向） 再学逆合成？（一方面正向反应预测的数据集大一些？另一方面人类也是这么个思路）反应预测数据集：USPTO-Mixed   逆合成：USPTO-50K

11.是否可以采用COT（乃至GOT）技术 如借助LLM做逆合成 可以让model先分析分子性质 与可能涉及的反应等。比如先问模型XX+XX可以合成给定分子的某一结构，或者是Molecule Captioning 分子提示  给定分子 先描述其性质？思维链作为提示能够进一步提高性能，在大模型（如5400亿参数的PaLM）上表现出最大增益，但是当模型参数规模小于1000亿时，思维链并不能产生性能增益，甚至可能会导致性能下降。因此，思维链引发的可能是模型规模上的“涌现”能力，即一种仅存在于大规模模型中的能力。

12.可以借助QSAR建模等将分子smiles式设计为2D\3D？

13.可以考虑数据增强方法——State-of-the-art augmented NLP transformer models for direct and single-step retrosynthesis

14.因为数据集小可以借助对比学习？借助其他模型产生难负例注意到很多模型的TOP1精准度在50左右  能利用上负例的话相当于样本扩充百分之50

15.Brody 等人（2021 年）提出了 GATv2，以更具表现力的方式改进注意力机制

「宁作吾」:
OGNN：可以将侧重点放在优化分子（原子表征提取  参考AttenFP?）

「宁作吾」:
化学知识的学习也是先学性质 在根据性质去推测反应  可以参考BERT的训练方法（MLM，比如将分子中各个部分切片，即仿照将分子看作自然语言）让图神经网络模型先学性质？

「宁作吾」:
切片方式：按官能团、链、环等

「宁作吾」:
OGNN用于无模板生成？（或许这是未来方向，大模型具有“涌现”能力）

「宁作吾」:
假如可行 可以引入3D建模  官能团间的距离可以引入位置编码

「宁作吾」:
KANO[44]方法引入了面向化学元素的知识图谱，以封装元素和官能团的基础知识                                    同样，PGMG[43]是一种以药理为导向的深度学习方法，它创新性地解决了药理和分子之间的映射难题，从而增加了生成的生物活性分子的多样性           [43] H. Zhu, R. Zhou, D. Cao, J. Tang, and M. Li, “A pharmacophoreguided deep learning approach for bioactive molecular generation,” Nature Communications, vol. 14, no. 1, p. 6234, 2023.
[44] Y. Fang, Q. Zhang, N. Zhang, Z. Chen, X. Zhuang, X. Shao, X. Fan,
and H. Chen, “Knowledge graph-enhanced molecular contrastive
learning with functional prompt,” Nature Machine Intelligence, pp.
1–12, 2023
...
「宁作吾」:
InstructMol（Cao等人，2023年）引入了分子图编码器，为Vicuna（Chiang等人， 2023 年 ） 编 码 二 维 分 子 图 信 息

「宁作吾」:
总结一下扩充数据集的方式，以及从数据集的扩充具有哪些方面的意义（Mol-ins可以增强对分子结构的理解）

「宁作吾」:
可以做一个多模态“小模型”？  input需要是多模态的（因为需要考虑文本对分子的描述）   训练过程中可以加入难负例对比学习（参考某个多模态模型）？
...
可以循序渐进  具备一些基础知识（比如可以是自然语言 可以是化学基础知识（如周期表、官能团等） 可以是文本描述的化学知识（如MOL-INS数据集））   在生物方面这一思路已经得到了运用（BioT5+）
